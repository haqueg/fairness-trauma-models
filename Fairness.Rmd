...
prediction models are ... <!-- List the major ones here, keep it to
1.5 lines -->

<!-- Use one paragraph (5 lines) to elaborate on these models-->

<!-- Move the most important parts of these two paragraphs to the list
of the most commonly used models and to the paragraph that elaborates
on those.

Indices such as the Injury Severity Score (ISS),
the New Injury Severity Score (NISS) and Trauma Mortality Prediction
Model (TMPM) are based on anatomic region of injury [@Haider2012]. The
ISS and NISS employ the Abbreviated Injury Scale (AIS) which assigns
scores based on region and type of anatomic structure injured and the
severity of the injury. The TMPM can be based either on the AIS or the
...
independent t-tests. We also compared differences in mean absolute
